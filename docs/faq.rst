...
Communication with the remote server (using an ssh: repo URL) happens via the SSH
protocol. This can lead to some issues that would not occur during a local backup:
...
- On the other hand, dropped packets or network issues may lead to a severed connection
  that is only noticed after some time by the server, leading to stale ``borg serve``
