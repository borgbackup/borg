...
Cryptography in |project_name|
==============================

Attack model
------------

The attack model of |project_name| is that the environment of the client process
...
Under these circumstances |project_name| guarantees that the attacker cannot
...
|project_name| is fundamentally based on an object graph structure (see :ref:`internals`),
where the root object is called the manifest.

|project_name| follows the `Horton principle`_, which states that
...
|project_name| is a MAC of the object's plaintext, therefore this ensures that
...
therefore |project_name| includes a tertiary authentication mechanism (TAM) that is applied
...
to add, remove or modify any part of the DAG without |project_name| being able to detect
the tampering.
...
because |project_name| will abort access when BORG_PASSPRHASE is incorrect.
...
.. [#] The reason why the authentication tag is stored in the packed
       data itself is that older |project_name| versions can still read the
...
|project_name| does not support unauthenticated encryption -- only authenticated encryption
...
and the repository before any data is encrypted. |project_name| uses its
...
said trusted channel. |project_name| does not have this functionality.
...
.. [#] Using the :ref:`borg key migrate-to-repokey <borg_key_migrate-to-repokey>`
       ...
       |project_name| does not support "passphrase" mode otherwise any more.
...
|project_name| cannot secure the key material while it is running, because the keys
...
- SHA-256 and SHA-512 from Python's hashlib_ standard library module are used.
  |project_name| requires a Python built with OpenSSL support (due to PBKDF2), therefore
...
SSH server -- |project_name| RPC does not contain *any* networking
code. Networking is done by the SSH client running in a separate
process, |project_name| only communicates over the standard pipes (stdout,
stderr and stdin) with this process. This also means that |project_name| doesn't
...
(cryptographic) network protocol |project_name| sidesteps many security issues
...
|project_name| does not use msgpack-c.
...
|project_name| uses the OpenSSL library for most cryptography (see `Implementations used`_ above).
...
however, it is important to note that |project_name| links against ``libcrypto`` **not** ``libssl``.
libcrypto is the low-level cryptography part of OpenSSL,
while libssl implements TLS and related protocols.

The latter is not used by |project_name| (cf. `Remote RPC protocol security`_, |project_name| itself does not implement
