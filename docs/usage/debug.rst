...
|project_name| has a ``--debug-topic TOPIC`` option to enable specific debugging messages. Topics
...
The ``borg debug profile-convert`` command can be used to take a |project_name| profile and convert
