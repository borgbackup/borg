...
def analyze_chunker(chunker_class, name, data, min_exp, max_exp, mask_bits, winsize, seed=0):
...
def plot_chunk_size_histogram(buzhash_stats, buzhash64_stats, output_file=None):
    ...
    plt.hist(buzhash_stats["sizes"], bins=bins, alpha=0.5, label="BuzHash")
    plt.hist(buzhash64_stats["sizes"], bins=bins, alpha=0.5, label="BuzHash64")
...
def plot_metrics_comparison(buzhash_stats, buzhash64_stats, output_file=None):
    ...
    plt.bar(x - width / 2, [v[0] for v in normalized_values], width, label="BuzHash")
    plt.bar(x + width / 2, [v[1] for v in normalized_values], width, label="BuzHash64")
...
def plot_bucket_distribution(buzhash_dist, buzhash64_dist, buzhash_stats, buzhash64_stats, output_file=None):
    ...
    plt.bar(x - width / 2, bh_pcts, width, label="BuzHash")
    plt.bar(x + width / 2, bh64_pcts, width, label="BuzHash64")
...
def main():
    ...
    if args.directory:
        ...
        buzhash_stats = analyze_chunker(Chunker, "BuzHash", data, min_exp, max_exp, mask_bits, winsize)
        buzhash64_stats = analyze_chunker(ChunkerBuzHash64, "BuzHash64", data, min_exp, max_exp, mask_bits, winsize)
    ...
    if buzhash_stats and buzhash64_stats:
        ...
        print("  Size Bucket | BuzHash Count (%) | BuzHash64 Count (%)")
        print("  -----------|-------------------|------------------")
