    ...
    if not create and (manifest or cache):
        ...
        if compatibility is not None:
            raise AssertionError(
                "with_repository called with compatibility argument but would not check" + repr(compatibility)
            )
        if create:
...
class Archiver:
    ...
    def do_benchmark_cpu(self, args):
        ...
        size = "1GB"
        tests = [("xxh64", lambda: xxh64(random_10M))]
        ...
        if not is_libressl:
            tests.extend(
                [
                    (
                        "aes-256-ocb",
                        lambda: AES256_OCB(None, key_256, iv=key_96, header_len=1, aad_offset=1).encrypt(
                            random_10M, header=b"X"
                        ),
                    ),
                    (
                        "chacha20-poly1305",
                        lambda: CHACHA20_POLY1305(None, key_256, iv=key_96, header_len=1, aad_offset=1).encrypt(
                            random_10M, header=b"X"
                        ),
                    ),
                ]
        ...
        print("Compression ====================================================")
        for spec in ["lz4", "zstd,1", "zstd,3", "zstd,22", "zlib,0", "zlib,6", "zlib,9", "lzma,0", "lzma,6", "lzma,9"]:
        ...
        if not dry_run:
            with Cache(
                ...
                metadata_collector = MetadataCollector(
                    noatime=not args.atime,
                    noctime=args.noctime,
                    noflags=args.nobsdflags or args.noflags,
                    noacls=args.noacls,
    ...
    def do_extract(self, args, repository, manifest, key, archive):
        ...
        for item in archive.iter_items(
            filter, partial_extract=partial_extract, preload=True, hardlink_masters=hardlink_masters
        ):
            orig_path = item.path
            if strip_components:
    ...
    def _export_tar(self, args, archive, tarstream):
        ...
        def item_content_stream(item):
            ...
            chunk_iterator = archive.pipeline.fetch_many(
                [chunk_id for chunk_id, _, _ in item.chunks], is_preloaded=True
            )
            if pi:
    ...
    def do_diff(self, args, repository, manifest, key, archive):
        ...
        archive1 = archive
        archive2 = Archive(repository, key, manifest, args.archive2, consider_part_files=args.consider_part_files)
            ...
            if args.forced == 0:  # without --force, we let the user see the archives list and confirm.
                ...
                try:
                    ...
                    if self.output_list:
                        ...
                        msg.append(
                            "You requested to completely DELETE the repository *including* %d archives it contains."
    ...
    def _info_repository(self, args, repository, manifest, key, cache):
        ...
        if args.json:
            ...
            """
                ...
                .strip()
                .format(id=bin_to_hex(repository.id), location=repository._location.canonical_path(), **info)
            )
            print(DASHES)
            print(STATS_HEADER)
            print(str(cache))
    ...
    def do_debug_dump_archive_items(self, args, repository, manifest, key):
        ...
        archive = Archive(
            repository, key, manifest, args.location.archive, consider_part_files=args.consider_part_files
        )
    ...
    def build_parser(self):
        ...
        **one can end up with truncated output being backed up**. Using
        ...
        fs_group.add_argument(
            "--numeric-owner", dest="numeric_ids", action="store_true", help="deprecated, use ``--numeric-ids`` instead"
        )
        fs_group.add_argument(
            "--numeric-ids",
            dest="numeric_ids",
        ...
        fs_group.add_argument("--noatime", dest="noatime", action="store_true", help="do not store atime into archive")
        ...
        fs_group.add_argument(
            "--nobsdflags", dest="nobsdflags", action="store_true", help="deprecated, use ``--noflags`` instead"
        )
        fs_group.add_argument(
            "--noflags",
            dest="noflags",
        ...
        fs_group.add_argument(
            ...
            action=Highlander,
            type=FilesCacheMode,
            default=DEFAULT_FILES_CACHE_MODE_UI,
        ...
        subparser.add_argument(
            "location", metavar="ARCHIVE", type=location_validator(archive=True), help="archive to dump"
        )
        ...
        subparser.add_argument(
            "location", metavar="REPOSITORY", type=location_validator(archive=False), help="repository to dump"
        )
        subparser.add_argument("path", metavar="PATH", type=str, help="file to dump data into")
        ...
        subparser.add_argument(
            "location", metavar="REPOSITORY", type=location_validator(archive=False), help="repository to search"
        )
        subparser.add_argument(
            "wanted",
            metavar="WANTED",
        ...
        subparser.add_argument(
            "location", metavar="REPOSITORY", type=location_validator(archive=False), help="repository to use"
        )
        ...
        subparser.add_argument("archives", metavar="ARCHIVE", nargs="*", help="archives to delete")
        define_archive_filters_group(subparser)
        ...
        subparser.add_argument(
            "location", metavar="ARCHIVE", type=location_validator(archive=True), help="archive to export"
        )
        ...
        subparser.add_argument(
            "location", metavar="ARCHIVE", type=location_validator(archive=True), help="archive to extract"
        )
        subparser.add_argument(
            "paths", metavar="PATH", nargs="*", type=str, help="paths to extract; patterns are supported"
        )
        ...
        subparser.add_argument(
            "location", metavar="REPOSITORY", nargs="?", default="", type=location_validator(archive=False)
        )
        ...
        **USE WITH CAUTION.**
        ...
        subparser.add_argument(
            "location", metavar="ARCHIVE", type=location_validator(archive=True), help="archive to rename"
        )
        subparser.add_argument(
            "name", metavar="NEWNAME", type=archivename_validator(), help="the new archive name to use"
        )
        ...
        subparser.add_argument(
            ...
            dest="inplace",
            action="store_true",
            help="rewrite repository in place, with no chance of going back " "to older versions of the repository.",
        )
        subparser.add_argument("--force", dest="force", action="store_true", help="Force upgrade")
        subparser.add_argument(
            "--tam",
            dest="tam",
            action="store_true",
        ...
        subparser.add_argument(
            "location", metavar="REPOSITORY", type=location_validator(archive=False), help="repository to lock"
        )
