...
class Archiver:
    ...
    def do_benchmark_crud(self, args):
        ...
        def measurement_run(args, path):
            ...
            rc = self.do_create(parse_args(args, ['create', compression, '--files-cache=disabled', archive + '1', path]))
            t_end = time.monotonic()
            dt_create = t_end - t_start
            assert rc == 0
            # now build files cache
            rc1 = self.do_create(parse_args(args, ['create', compression, archive + '2', path]))
            rc2 = self.do_delete(parse_args(args, ['delete', archive + '2']))
            assert rc1 == rc2 == 0
            # measure a no-change update (archive1 is still present)
            t_start = time.monotonic()
            rc1 = self.do_create(parse_args(args, ['create', compression, archive + '3', path]))
            t_end = time.monotonic()
            dt_update = t_end - t_start
            rc2 = self.do_delete(parse_args(args, ['delete', archive + '3']))
            assert rc1 == rc2 == 0
            # measure extraction (dry-run: without writing result to disk)
            t_start = time.monotonic()
            rc = self.do_extract(parse_args(args, ['extract', '--dry-run', archive + '1']))
            t_end = time.monotonic()
            dt_extract = t_end - t_start
            assert rc == 0
            # measure archive deletion (of LAST present archive with the data)
            t_start = time.monotonic()
            rc = self.do_delete(parse_args(args, ['delete', archive + '1']))
            t_end = time.monotonic()
