...
def _report_time(report):
    ...
    Unsigned reports from an unencrypted repo carry whatever the publisher - or the
...
class MonitorMixIn:
    ...
    def _monitor_read(self, args, repository):
        ...
        for entry in entries:
            if entry["stale"] or entry["report"].get("status") == "error":
                set_ec(EXIT_ERROR)
            elif entry["report"].get("status") == "warning" or not entry["trusted"]:
                set_ec(EXIT_WARNING)
    ...
    def _monitor_output(self, args, entries):
        ...
        for e in entries:
            ...
            hint = "" if e["trusted"] else "  (unsigned - no BORG_MONITORING_KEY set, or repo is unencrypted)"
