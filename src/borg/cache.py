...
class Cache:
    ...
    def __new__(
        ...
        cache_mode=DEFAULT_FILES_CACHE_MODE,
...
Chunk index:    {0.total_unique_chunks:20d} {0.total_chunks:20d}"""
    ...
    Summary = namedtuple(
        "Summary", ["total_size", "total_csize", "unique_size", "unique_csize", "total_unique_chunks", "total_chunks"]
    )
    ...
    def stats(self):
        ...
        stats = self.Summary(
            total_size, total_csize, unique_size, unique_csize, total_unique_chunks, total_chunks
        )._asdict()
        return stats
        ...
        if self.files is not None:
            ...
            with IntegrityCheckedFile(path=os.path.join(self.path, files_cache_name()), write=True) as fd:
                ...
                for path_hash, item in self.files.items():
                    ...
                    if (
                        entry.age == 0
                        and bigint_to_int(entry.cmtime) < self._newest_cmtime
                        or entry.age > 0
                        and entry.age < ttl
                    ):
    ...
    def sync(self):
        ...
        def create_master_idx(chunk_idx):
            ...
            if archive_ids:
                ...
                pi.finish()
                logger.debug(
                    "Cache sync: had to fetch %s (%d chunks) because no archive had a csize set for them "
                    "(due to --no-cache-sync)",
                    format_file_size(fetched_bytes_for_csize),
                    fetched_chunks_for_csize,
                )
                logger.debug(
