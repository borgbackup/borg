...
cdef extern from "_chunker.c":
    ...
    uint32_t c_buzhash_start(uint8_t *data, size_t len, uint32_t *h)
    uint32_t c_buzhash_update(uint32_t sum, uint8_t remove, uint8_t add, size_t len, uint32_t *h)
    uint32_t c_buzhash_find(uint8_t *data, size_t len, size_t min_size, size_t max_size,
                            size_t wsize, uint32_t mask, uint32_t *h)
...
cdef class Chunker:
    ...
    This chunker makes quite some effort to mostly cut chunks of the same content, even if
    ...
    cdef readonly int bytes_read, bytes_yielded
    cdef uint32_t table[256]
    cdef uint32_t hash_mask
    cdef size_t window_size, min_size, max_size
    cdef readonly object reader
    ...
    def __cinit__(self, int seed, int chunk_min_exp, int chunk_max_exp, int hash_mask_bits, int hash_window_size):
        ...
        buzhash_init_table(self.table, seed & 0xffffffff)
        self.chunking_time = 0.0
        self.reader = FileReader(1024 * 1024, sparse=False)

    def get_data(self, block_iter):
    ...
    def make_chunk(self, data):
        data_size = len(data)
        self.bytes_yielded += data_size
        assert len(zeros) >= data_size
        if zeros.startswith(data):
            data = None
            allocation = CH_ALLOC
        else:
            allocation = CH_DATA
        return Chunk(data, size=data_size, allocation=allocation)
    ...
    def chunkify(self, fd=None, fh=-1, fmap=None):
        ...
        block_iter = self.reader.blockify(fd, fh, fmap)
        data = b""
        need_more_data = True
        while True:
            while need_more_data or len(data) <= self.min_size + self.window_size:
                try:
                    more_data = self.get_data(block_iter)
                except StopIteration:  # EOF
                    if len(data) > 0:
                        yield self.make_chunk(data)
                    assert self.bytes_read == self.bytes_yielded
                    return
                data += more_data
                need_more_data = False
            started_chunking = time.monotonic()
            # we start searching for cutting points at min_size as we don't want anything smaller anyway
            pos = c_buzhash_find(data, len(data), self.min_size, self.max_size, self.window_size, self.hash_mask, self.table)
            self.chunking_time += time.monotonic() - started_chunking
            if pos == 0xffffffff:  # no cutting point found
                need_more_data = True
            else:
                yield self.make_chunk(data[:pos])
                data = data[pos:]
...
def buzhash(data, unsigned long seed):
    cdef uint32_t table[256]
    buzhash_init_table(table, seed & 0xffffffff)
    return c_buzhash_start(<const uint8_t *> data, len(data), table)


def buzhash_update(uint32_t sum, uint8_t remove, uint8_t add, size_t len, unsigned long seed):
    cdef uint32_t table[256]
    buzhash_init_table(table, seed & 0xffffffff)
    return c_buzhash_update(sum, remove, add, len, table)


cdef void buzhash_init_table(uint32_t *table, uint32_t seed):
    table_base = [
    ...
    assert len(table_base) == 256
    for i in range(256):
        table[i] = <uint32_t> table_base[i] ^ seed
