...
cdef class CSPRNG:
    ...
    def random_int(self, n):
        ...
        cdef uint64_t upper_bound = n

        # Calculate how many bytes we need
        cdef size_t bytes_needed = 0
        cdef uint64_t temp = upper_bound
        while temp > 0:
            bytes_needed += 1
            temp >>= 8
        ...
        cdef bytes random_data
        cdef uint64_t result
        cdef uint64_t mask = (1 << (bytes_needed * 8)) - 1
        ...
        while True:
            ...
            for i in range(bytes_needed):
                result = (result << 8) | random_data[i]

            # Apply mask to get the right number of bits
            result &= mask

            # If result is in range, return it
