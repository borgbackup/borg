...
For the ``none-*`` (unkeyed) modes there is no secret to derive from, so reports
...
def is_signed_repo(key):
    ...
    # the "none-*" modes have no secret key material: derive_key() would return something
    # anybody can recompute, so a "signature" from it would authenticate nothing.
    return key.has_secret_key
