...
class FuseOperations(llfuse.Operations, FuseBackend):
    ...
    def mount(self, mountpoint, mount_options, foreground=False, show_rc=False):
        ...
        logger.warning('ATTENTION: The mounted archive may contain symlinks that point outside the archive tree. \nWhen following such symlinks you may see files within the mountpoint that are not part of the archive,or different versions of files than those stored in the archive.')
        if not foreground:
