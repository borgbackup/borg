...
from .crypto import key as crypto_key
from .crypto.key import is_keyfile, store_hash, STORE_HASH_NAME

logger = create_logger(__name__)

# an object name is the hex store hash of the object's content (64 lowercase hex digits), see
# crypto.key.store_hash(). borgstore names the packs it writes for us (defrag) and verifies objects
# (hash) with the store hash, which it knows by STORE_HASH_NAME.
...
class PackWriter:
    ...
    def _store_pieces(self, pieces, outcome, trace=False):
        ...
        try:
            ...
            # Name the pack by the store hash of its bytes: the name commits to the stored content,
            # so borgstore can verify and cache the file.
            self._trace("H", trace)
            pack_id = store_hash(pack_data).digest()
...
SALVAGE_UNSTABLE = "unstable"  # two reads of the pack gave different results
SALVAGE_READ_ERROR = "read error"  # reading the pack raised OSError
...
class Repository:
    ...
    def __init__(
        ...
        # True if BORG_STORE_CACHE set up a local cache of packs: store.load() of a pack may then return the
        # cached copy instead of reading the backend.
        self.pack_store_cache = cache_url is not None
    ...
    def store_key(self, keydata):
        ...
        digest = store_hash(keydata).hexdigest()
    ...
    def check(self, repair=False, max_duration=0, max_age=0, repo_only=False, validate=None):
        ...
        def verify(namespace, name):
            ...
            try:
                ok = self.store.hash(key, algorithm=STORE_HASH_NAME) == name
            except StoreObjectNotFound:
                return True  # vanished since store.list(); not an error
            if not ok:
                logger.error(f"Store object {key} is corrupted: content does not match its name (store hash).")
            return ok
    ...
    def compact_pack(
        ...
        # write the new pack (named by the store hash of its content) from those spans before touching the index
        ...
        if sources:
            try:
                new_pack_id = hex_to_bin(self.store.defrag(sources, algorithm=STORE_HASH_NAME, namespace="packs"))
    ...
    def merge_packs(self, pack_ids, *, chunks=None, max_size=None):
        ...
        # write each batch as a new pack (named by the store hash of its content) and repoint its objects: an
    ...
    def transform_pack(self, pack_id, ids, transform, *, validate, chunks=None, before_change=None):
        ...
        touched at all. Otherwise the new pack (named by the store hash of its content) is stored, the indexed
        ...
        new_pack_id = store_hash(pack_data).digest()
    ...
    def salvage_pack(self, pack_id, *, validate, authenticate, chunks, before_old_pack_delete=None):
        ...
        - SALVAGE_UNSTABLE: the loaded bytes hash to the pack's name, or a second load of the pack
          differs from the first. An object is dropped only if two identical loads both fail it.
        - SALVAGE_READ_ERROR: reading the pack raised OSError. All reads happen before the first
          store change.
        ...
        Raises Error before any store access if BORG_WORKAROUNDS=authenticated_no_key is set (then
        decrypting skips the tag verification, so authenticate accepts damaged objects) or if
        pack_store_cache is set (then two loads can return the same cached copy). Raises
        PermissionDenied unless the repo permissions allow compaction (see assert_writable), and
        StoreObjectNotFound if the pack is missing. Requires the exclusive lock.
        """
        if crypto_key.AUTHENTICATED_NO_KEY:
            raise Error(
                "Pack salvage refused: with BORG_WORKAROUNDS=authenticated_no_key objects are not authenticated."
            )
        if self.pack_store_cache:
        ...
        try:
            if self.store.hash(pack_key, algorithm=STORE_HASH_NAME) == pack_hex:
                return untouched(SALVAGE_INTACT)
            pack_contents = self.store.load(pack_key)
            reader = PackReader(pack_id=pack_id, pack_contents=pack_contents)
