...
class LoggedIO:
    ...
    def iter_objects(self, segment, offset=0, include_data=False, read_data=True):
        ...
        while header:
            size, tag, key, data = self._read(
                fd, self.header_fmt, header, segment, offset, (TAG_PUT, TAG_DELETE, TAG_COMMIT), read_data=read_data
            )
            if include_data:
    ...
    def _read(self, fd, fmt, header, segment, offset, acceptable_tags, read_data=True):
        ...
        try:
            ...
            raise IntegrityError(
                "Invalid segment entry header [segment {}, offset {}]: {}".format(segment, offset, err)
            ) from None
        ...
        if size > MAX_OBJECT_SIZE:
            ...
            raise IntegrityError(
                "Invalid segment entry size {} - too big [segment {}, offset {}]".format(size, segment, offset)
            )
        if size < fmt.size:
            raise IntegrityError(
                "Invalid segment entry size {} - too small [segment {}, offset {}]".format(size, segment, offset)
            )
        ...
        if read_data:
            ...
            if len(data) != length:
                raise IntegrityError(
                    "Segment entry data short read [segment {}, offset {}]: expected {}, got {} bytes".format(
                        segment, offset, length, len(data)
                    )
                )
            if crc32(data, crc32(memoryview(header)[4:])) & 0xFFFFFFFF != crc:
                raise IntegrityError("Segment entry checksum mismatch [segment {}, offset {}]".format(segment, offset))
            ...
            if seeked != length:
                raise IntegrityError(
                    "Segment entry data short seek [segment {}, offset {}]: expected {}, got {} bytes".format(
                        segment, offset, length, seeked
                    )
                )
        if tag not in acceptable_tags:
            raise IntegrityError(
                "Invalid segment entry header, did not get acceptable tag [segment {}, offset {}]".format(
                    segment, offset
                )
            )
