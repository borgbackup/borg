...
class ArchiverTestCase(ArchiverTestCaseBase):
    ...
    def test_basic_functionality(self):
        ...
        output = self.cmd(
            "init", "--encryption=repokey", "--show-version", "--show-rc", self.repository_location, fork=True
        )
    ...
    def test_fuse_mount_hardlinks(self):
        ...
        with self.fuse_mount(
            self.repository_location + "::test", mountpoint, "--strip-components=2", *ignore_perms
        ...
        with self.fuse_mount(self.repository_location + "::test", mountpoint, "input/dir1", *ignore_perms), changedir(
            mountpoint
        ):
        ...
        with self.fuse_mount(self.repository_location + "::test", mountpoint, *ignore_perms), changedir(mountpoint):
    ...
    def test_create_paths_from_command(self):
        ...
        self.cmd("create", "--paths-from-command", self.repository_location + "::test", "--", "echo", input_data)
        archive_list = self.cmd("list", "--json-lines", self.repository_location + "::test")
        paths = [json.loads(line)["path"] for line in archive_list.split("\n") if line]
    ...
    def test_create_pattern_intermediate_folders_first(self):
        ...
        with changedir("input"):
            self.cmd("create", "--patterns-from=" + self.patterns_file_path2, self.repository_location + "::test", ".")
    ...
    def test_create_no_cache_sync(self):
        ...
        create_json = json.loads(
            self.cmd("create", "--no-cache-sync", self.repository_location + "::test", "input", "--json", "--error")
    ...
    def test_file_status_rc_cache_mode(self):
        ...
        self.cmd("init", "--encryption=repokey", self.repository_location)
        output = self.cmd(
            "create", "--list", "--files-cache=rechunk,ctime", self.repository_location + "::test1", "input"
        )
        # no changes here, but this mode rechunks unconditionally
        output = self.cmd(
            "create", "--list", "--files-cache=rechunk,ctime", self.repository_location + "::test2", "input"
        )
        self.assert_in("A input/file1", output)
    ...
    def test_file_status_excluded(self):
        ...
        output = self.cmd(
            ...
            "--list",
            "--exclude-nodump",
            self.repository_location + "::test1",
            "input",
            "--exclude",
            "*/file2",
        )
    ...
    def test_prune_retain_and_expire_oldest(self):
        ...
        output = self.cmd(
            "prune", "--list", "--dry-run", self.repository_location, "--keep-daily=7", "--keep-monthly=1"
        )
    ...
    def test_prune_repository_glob(self):
        ...
        output = self.cmd(
            "prune", "--list", "--dry-run", self.repository_location, "--keep-daily=1", "--glob-archives=2015-*-foo"
        )
    ...
    def test_list_format(self):
        ...
        output_2 = self.cmd(
            "list", "--format", "{mode} {user:6} {group:6} {size:8d} {mtime} {path}{extra}{NEWLINE}", test_archive
        )
    ...
    def test_fuse(self):
        ...
        with self.fuse_mount(self.repository_location + "::archive", mountpoint):
            self.assert_dirs_equal(
                self.input_path, os.path.join(mountpoint, "input"), ignore_flags=True, ignore_xattrs=True
            )
            # regular file
            in_fn = "input/file1"
    ...
    def test_recreate_rechunkify(self):
        ...
        list = self.cmd(
            "list", self.repository_location + "::test1", "input/large_file", "--format", "{num_chunks} {unique_chunks}"
        )
    ...
    def test_recreate_recompress(self):
        ...
        file_list = self.cmd(
            "list", self.repository_location + "::test", "input/compressible", "--format", "{size} {csize} {sha256}"
        )
    ...
    def test_recreate_timestamp(self):
        ...
        archive = self.repository_location + "::test0"
        self.cmd("create", archive, "input")
        self.cmd("recreate", "--timestamp", "1970-01-02T00:00:00", "--comment", "test", archive)
...
"""
    ...
    def test_export_tar_strip_components(self):
        ...
        self.cmd("create", self.repository_location + "::test", "input")
        list = self.cmd(
            "export-tar", self.repository_location + "::test", "simple.tar", "--strip-components=1", "--list"
        )
...
def test_get_args():
    ...
    args = archiver.get_args(
        ["borg", "serve", "--restrict-to-path=/p1", "--restrict-to-path=/p2"], "borg init --encryption=repokey /"
    )
    assert args.func == archiver.do_serve
