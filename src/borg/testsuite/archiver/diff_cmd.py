...
def test_basic_functionality(archiver_setup, create_regular_file, cmd_fixture, assert_line_exists):
    repo_location = archiver_setup.repository_location
    # Setup files for the first snapshot
    create_regular_file("empty", size=0)
    create_regular_file("file_unchanged", size=128)
    create_regular_file("file_removed", size=256)
    create_regular_file("file_removed2", size=512)
    create_regular_file("file_replaced", size=1024)
    ...
    cmd_fixture(f"--repo={repo_location}", "create", "test0", "input")

    # Setup files for the second snapshot
    create_regular_file("file_added", size=2048)
    create_regular_file("file_empty_added", size=0)
    os.unlink("input/file_replaced")
    create_regular_file("file_replaced", contents=b"0" * 4096)
    os.unlink("input/file_removed")
    os.unlink("input/file_removed2")
    os.rmdir("input/dir_replaced_with_file")
    create_regular_file("dir_replaced_with_file", size=8192)
    ...
    if are_symlinks_supported():
        ...
        create_regular_file("link_replaced_by_file", size=16384)
    ...
    cmd_fixture(f"--repo={repo_location}", "create", "test1a", "input")
    cmd_fixture(f"--repo={repo_location}", "create", "test1b", "input", "--chunker-params", "16,18,17,4095")
    ...
    output = cmd_fixture(f"--repo={repo_location}", "diff", "test0", "test1a")
    do_asserts(output, True)
    # We expect exit_code=1 due to the chunker params warning
    output = cmd_fixture(f"--repo={repo_location}", "diff", "test0", "test1b", "--content-only", exit_code=1)
    do_asserts(output, False, content_only=True)

    output = cmd_fixture(f"--repo={repo_location}", "diff", "test0", "test1a", "--json-lines")
    do_json_asserts(output, True)

    output = cmd_fixture(f"--repo={repo_location}", "diff", "test0", "test1a", "--json-lines", "--content-only")
    do_json_asserts(output, True, content_only=True)


def test_time_diffs(archiver_setup, create_regular_file, cmd_fixture):
    repo_location = archiver_setup.repository_location
    cmd_fixture(f"--repo={repo_location}", "rcreate", archiver_setup.RK_ENCRYPTION)
    create_regular_file("test_file", size=10)
    cmd_fixture(f"--repo={repo_location}", "create", "archive1", "input")
    time.sleep(0.1)
    ...
    create_regular_file("test_file", size=15)
    cmd_fixture(f"--repo={repo_location}", "create", "archive2", "input")
    output = cmd_fixture(
        f"--repo={repo_location}", "diff", "archive1", "archive2", "--format", "'{mtime}{ctime} {path}{NL}'"
    )
    ...
    cmd_fixture(f"--repo={repo_location}", "create", "archive3", "input")
    output = cmd_fixture(
        f"--repo={repo_location}", "diff", "archive2", "archive3", "--format", "'{mtime}{ctime} {path}{NL}'"
    )
...
def test_sort_option(archiver_setup, create_regular_file, cmd_fixture):
    repo_location = archiver_setup.repository_location
    cmd_fixture(f"--repo={repo_location}", "rcreate", archiver_setup.RK_ENCRYPTION)

    create_regular_file("a_file_removed", size=8)
    create_regular_file("f_file_removed", size=16)
    create_regular_file("c_file_changed", size=32)
    create_regular_file("e_file_changed", size=64)
    cmd_fixture(f"--repo={repo_location}", "create", "test0", "input")
    ...
    create_regular_file("c_file_changed", size=512)
    create_regular_file("e_file_changed", size=1024)
    create_regular_file("b_file_added", size=128)
    create_regular_file("d_file_added", size=256)
    cmd_fixture(f"--repo={repo_location}", "create", "test1", "input")

    output = cmd_fixture(f"--repo={repo_location}", "diff", "test0", "test1", "--sort", "--content-only")
