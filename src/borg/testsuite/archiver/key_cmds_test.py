...
def test_keyfile_name_is_content_store_hash(archivers, request):
    archiver = request.getfixturevalue(archivers)
    cmd(archiver, "repo-create", KF_ENCRYPTION, KF_LOCATION)
    [key_filename] = os.listdir(archiver.keys_path)
    ...
    with open(key_path, "rb") as fd:
        key_content = fd.read()
    assert key_filename == store_hash(key_content).hexdigest()


def test_change_passphrase_renames_keyfile_to_new_store_hash(archivers, request):
    archiver = request.getfixturevalue(archivers)
    cmd(archiver, "repo-create", KF_ENCRYPTION, KF_LOCATION)
    ...
    with open(new_key_path, "rb") as fd:
        key_content = fd.read()
    assert new_key_filename == store_hash(key_content).hexdigest()
    cmd(archiver, "repo-list")
