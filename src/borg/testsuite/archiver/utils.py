...
from borg.constants import EXIT_SUCCESS, CACHE_TAG_NAME, CACHE_TAG_CONTENTS, ISO_FORMAT, EXIT_ERROR
from borg.helpers import bin_to_hex, Location, umount
from borg.logger import flush_logging
from borg.manifest import Manifest
from borg.platform import get_flags
...
from borg.xattr import get_all
...
def cmd(archiver, *args, **kw):
    ...
    pp_msg = archiver.PURE_PYTHON_MSGPACK_WARNING.encode() if binary_output else archiver.PURE_PYTHON_MSGPACK_WARNING
...
def create_src_archive(archiver, name, ts=None):
    repo_location, source_dir = archiver.repository_location, archiver.src_dir
    if ts:
...
def open_repository(archiver):
    if archiver.prefix == "":
        ...
        print(f"{archiver.prefix} is not a valid prefix! Cannot open repo.")
