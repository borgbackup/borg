...
class TestCacheSynchronizer:
    ...
    def test_simple(self, index, sync):
        data = packb({"foo": "bar", "baz": 1234, "bar": 5678, "user": "chunks", "chunks": [(H(1), 1, 2), (H(2), 2, 3)]})
        sync.feed(data)
        assert len(index) == 2
        ...
        data += packb(
            {
                ...
                "chunks": [(H(1), 1, 2), (H(2), 2, 3)],
                "stuff": [(1, 2, 3)],
            }
        )
        data += packb({"chunks": [(H(3), 1, 2)]})
        data += packb({"chunks": [(H(1), 1, 2)]})
    ...
    @pytest.mark.parametrize(
        "structure",
        (lambda elem: {"chunks": elem}, lambda elem: {"chunks": [elem]}, lambda elem: {"chunks": [(elem, 1, 2)]}),
    )
