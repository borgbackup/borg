...
class TestLocationWithoutEnv:
    ...
    def test_ssh(self, monkeypatch, keys_dir):
        ...
        assert (
            repr(Location("ssh://user@[::]:1234/some/path::archive"))
        ...
        assert (
            repr(Location("ssh://user@[2001:db8::]:1234/some/path::archive"))
        ...
        assert (
            repr(Location("ssh://user@[2001:db8::c0:ffee]:1234/some/path::archive"))
        ...
        assert (
            repr(Location("ssh://user@[2001:db8::192.0.2.1]:1234/some/path::archive"))
    ...
    def test_scp(self, monkeypatch, keys_dir):
        ...
        assert (
            repr(Location("user@host:/some/path"))
            == "Location(proto='ssh', user='user', host='host', port=None, path='/some/path', archive=None)"
        )
        assert (
            repr(Location("user@[::]:/some/path::archive"))
        ...
        assert (
            repr(Location("user@[2001:db8::]:/some/path::archive"))
        ...
        assert (
            repr(Location("user@[2001:db8::c0:ffee]:/some/path::archive"))
        ...
        assert (
            repr(Location("user@[2001:db8::192.0.2.1]:/some/path::archive"))
        ...
        assert (
            Location("user@[2001:db8::192.0.2.1]:/some/path").to_key_filename()
            == keys_dir + "2001_db8__192_0_2_1__some_path"
        )
    ...
    def test_smb(self, monkeypatch, keys_dir):
        monkeypatch.delenv("BORG_REPO", raising=False)
        assert (
            repr(Location("file:////server/share/path::archive"))
    ...
    def test_user_parsing(self):
        # see issue #1930
        assert (
            repr(Location("host:path::2016-12-31@23:59:59"))
            == "Location(proto='ssh', user=None, host='host', port=None, path='path', archive='2016-12-31@23:59:59')"
        )
        assert (
            repr(Location("ssh://host/path::2016-12-31@23:59:59"))
            == "Location(proto='ssh', user=None, host='host', port=None, path='/path', archive='2016-12-31@23:59:59')"
        )

    def test_with_timestamp(self):
        assert (
            repr(Location("path::archive-{utcnow}").with_timestamp(datetime(2002, 9, 19, tzinfo=timezone.utc)))
    ...
    def test_canonical_path(self, monkeypatch):
        ...
        locations = [
            "some/path::archive",
            "file://some/path::archive",
            "host:some/path::archive",
            "host:~user/some/path::archive",
            "ssh://host/some/path::archive",
    ...
    def test_format_path(self, monkeypatch):
        ...
        test_pid = os.getpid()
        assert (
            repr(Location("/some/path::archive{pid}"))
...
class TestLocationWithEnv:
    ...
    def test_folder(self, monkeypatch):
        ...
        assert (
            repr(Location("::")) == "Location(proto='file', user=None, host=None, port=None, path='path', archive=None)"
        )
